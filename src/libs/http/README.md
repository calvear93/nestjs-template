...
export class UserService {
  constructor(
    ...
    @Inject('ANOTHER_API_CLIENT') private readonly anotherClient: HttpClient
  ) {}
}
```
...
export class UserService {
  constructor(
    @Inject('JSON_PLACEHOLDER_CLIENT') private readonly httpClient: HttpProvider
  ) {}
...
interface HttpClientConfig {
  ...
  // request interceptor
  onRequest?: (options: HttpRequestOptions, url: string | URL) => void | Promise<void>;
...
export class AuthService {
  ...
  async login(credentials: LoginDto): Promise<AuthResponse> {
    const response = await this.authClient.post<AuthResponse>('/auth/login', {
      data: credentials
    });
    return response.json();
  }
  ...
  async refreshToken(refreshToken: string): Promise<AuthResponse> {
    const response = await this.authClient.post<AuthResponse>('/auth/refresh', {
      data: { refreshToken }
    });
    return response.json();
  }
...
export class FileService {
  ...
  async uploadFile(file: File, metadata?: Record<string, any>): Promise<UploadResponse> {
    ...
    const response = await this.httpClient.post<UploadResponse>('/files/upload', {
...
export class ApiService {
  ...
  async getAllPages<T>(endpoint: string, filters?: Record<string, any>): Promise<T[]> {
    let allData: T[] = [];
    let page = 1;
    let totalPages = 1;

    do {
      const response = await this.getPaginatedData<T>(endpoint, page, 100, filters);
