...
```typescript
import { ZodDto, ZodIterableDto, ZodValidationPipe, epoch, phone } from '#libs/zod';
```
...
export class NumberArrayDto extends ZodIterableDto(NumberArraySchema, 'NumberArray') {}
...
const coordinate = new CoordinateDto([40.7128, -74.0060, 'New York']);
```
...
export class UserRegistrationDto extends ZodDto(UserRegistrationSchema, 'UserRegistration') {}
```
...
export class NotificationDto extends ZodDto(NotificationSchema, 'Notification') {}
```

### Array DTO with Validations

```typescript
const BulkUserUpdateSchema = z.array(
  z.object({
    id: z.string().uuid(),
    updates: z.object({
...
export class BulkUserUpdateDto extends ZodIterableDto(BulkUserUpdateSchema, 'BulkUserUpdate') {}
```
