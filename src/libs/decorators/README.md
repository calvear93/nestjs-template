...
```typescript
import { createSecurityGuard, SecurityGuard, ApplyToClass, ApplyToProperty } from '#libs/decorators';
```
...
export const [AdminRole, AdminAllowAnonymous] = createSecurityGuard(AdminRoleGuard);
export const [ApiKeyAuth, ApiKeyAllowAnonymous] = createSecurityGuard(ApiKeyGuard);
...
const createControllerDecorators = (): DecoratorsLookUp => ({
  ...
  common: {
    method: [
      UseGuards(process.env.NODE_ENV === 'production' ? ProductionGuard : DevGuard)
    ]
  }
});
...
export class PermissionGuard implements SecurityGuard {
  canActivate(
    ...
    // Ownership check
    if (requireOwnership) {
      return user?.id === resourceId || user?.ownedResources?.includes(resourceId);
    }
...
interface SecurityGuard {
  canActivate(context: ExecutionContext, ...args: any[]): Promise<boolean> | boolean;
}
```
...
export const [JwtAuth, JwtAllowAnonymous] = createSecurityGuard(JwtAuthGuard);
export const [BasicAuth, BasicAllowAnonymous] = createSecurityGuard(BasicAuthGuard);
```
